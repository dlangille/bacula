					...
					<td>
						...
						<script type="text/javascript">
							var <%=$this->ApplyAction->ClientID%>_actions_func = function() {
								var console_visible = $('#<%=$this->getPage()->Console->ConsoleContainer->ClientID%>').is(':visible');
	...
	<div class="slide-window-bar">
		...
		<com:TCallback ID="DetailView" OnCallback="switchView">
			<prop:ClientSide.OnLoading>
				$('#<%=$this->getParent()->getID()%>-slide-window-progress').css({'display': 'block'});
			</prop:ClientSide.OnLoading>
			<prop:ClientSide.OnComplete>
				$('#<%=$this->getParent()->getID()%>-slide-window-progress').css({'display': 'none'});
		...
		<script type="text/javascript">
			...
			$('input[id=<%=$this->Simple->ClientID%>], input[id=<%=$this->Details->ClientID%>], select[id=<%=$this->Limit->ClientID%>]').each(function(index, el) {
				$(el).on('change', <%=$this->getParent()->getID()%>_refresh_window_func);
			});
		</script>
	</div>
</div>
