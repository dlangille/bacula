	...
	<com:TActivePanel ID="Labeling" Display="None" Style="margin-left: 20px">
		...
		<com:TActivePanel ID="BarcodeSlotsField" Display="None">
			<div class="line">
				...
				<div class="field"><com:TActiveTextBox ID="SlotsLabel" CssClass="textbox" Text="0" /></div>
			</div>
		</com:TActivePanel>
		<div class="line">
...
<script type="text/javascript">
	$('#volumes_tools_launcher, #<%=$this->LabelButton->ClientID%>, #<%=$this->UpdateSlotsButton->ClientID%>, #<%=$this->UpdateSlotsScanButton->ClientID%>').each(function(index, el){
		$(el).on('click', function(){
			if(VolumesToolsValidation(el) === true) {
				$('#<%=$this->getPage()->Console->ConsoleContainer->ClientID%>').hide();
				$('#console_launcher').children('span')[0].textContent = '<%[ show console ]%>';
				$('#<%=$this->Tools->ClientID%>').slideToggle({'duration': 200, done: function() {
					window.scrollTo(0, document.body.scrollHeight);
				}});
			
				if(el.id != 'volumes_tools_launcher') {
					$('#<%=$this->getPage()->Console->ConsoleContainer->ClientID%>').slideToggle({'duration': 300, done: function(){
						window.scrollTo(0, document.body.scrollHeight);
					}});
				}
			}
	...
	function VolumesToolsValidation(el) {
		...
		if(el.id == '<%=$this->LabelButton->ClientID%>') {
			if(document.getElementById('<%=$this->Barcodes->ClientID%>').checked === true) {
				if(slotsPattern.test(document.getElementById('<%=$this->SlotsLabel->ClientID%>').value) === false) {
					var validation = false;
				}
			} else {
				if(labelNamePattern.test(document.getElementById('<%=$this->LabelName->ClientID%>').value) === false) {
					var validation = false;
				}
			}
			if(drivePattern.test(document.getElementById('<%=$this->DriveLabel->ClientID%>').value) === false) {
				var validation = false;
			}
		} else if(el.id == '<%=$this->UpdateSlotsButton->ClientID%>') {
			if(drivePattern.test(document.getElementById('<%=$this->DriveUpdateSlots->ClientID%>').value) === false || slotsPattern.test(document.getElementById('<%=$this->SlotsUpdateSlots->ClientID%>').value) === false) {
				var validation = false;
			}
		} else if(el.id == '<%=$this->UpdateSlotsScanButton->ClientID%>') {
			if(drivePattern.test(document.getElementById('<%=$this->DriveUpdateSlotsScan->ClientID%>').value) === false || slotsPattern.test(document.getElementById('<%=$this->SlotsUpdateSlotsScan->ClientID%>').value) === false) {
				var validation = false;
			}
		}
		return validation;
