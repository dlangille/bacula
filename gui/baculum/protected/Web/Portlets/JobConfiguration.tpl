		...
		<div id="job_console_tab" style="display: none"> 
			...
			<script type="text/javascript">
				...
				var status_callback_func = function() {
					if($('#<%=$this->getID()%>configuration').is(':visible') && (document.getElementById('<%=$this->RefreshStart->ClientID%>').value === 'true' || status_prev === true)) {
						status_prev = (document.getElementById('<%=$this->RefreshStart->ClientID%>').value === 'true');
