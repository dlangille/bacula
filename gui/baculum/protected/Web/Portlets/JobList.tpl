...
<com:TContent ID="SlideWindowContent">
	...
	>
		<com:TTemplateColumn HeaderText="<input type='checkbox' name='actions_checkbox' onclick=SlideWindow.getObj('JobWindow').markAllChecked(this.checked)>" ItemStyle.HorizontalAlign="Center">
			<prop:ItemTemplate>
	...
	<com:TCallback ID="RunJobCall" OnCallback="Page.JobWindow.run_again">
		<prop:ClientSide.OnLoading>
			...
			var img_btn = document.getElementById('run_job_again_btn');
			var img_src_path = img_btn.getAttribute('src').replace(/[^\/]+\S$/, '');
			img_btn.setAttribute('disabled', 'disabled');
			img_btn.setAttribute('src', img_src_path + 'loader.gif');
		</prop:ClientSide.OnLoading>
		<prop:ClientSide.OnComplete>
			var img_btn = document.getElementById('run_job_again_btn');
			var img_src_path = img_btn.getAttribute('src').replace(/[^\/]+\S$/, '');
			img_btn.setAttribute('src', img_src_path + 'play.png');
