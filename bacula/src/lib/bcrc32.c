...
// The text below is found on the above link:
// License
...
// Adapted to Bacula Enterprise by Kern Sibbald, June 2015
//
//  The modifications made for adapting to Bacula are minimum to fit in
//  with the Bacula calling sequences.  No functionality was changed.
...
//extern uint32_t old_bcrc32(unsigned char*buf, int len)
...
// define endianess and some integer data types
  ...
  #if defined(HAVE_FREEBSD_OS) || defined(HAVE_OPENBSD_OS) || defined(HAVE_NETBSD_OS)
     #include <sys/endian.h>
  #elif defined(HAVE_DARWIN_OS)
     #include <machine/endian.h>
  #elif defined(HAVE_SUN_OS)
     #include <sys/isa_defs.h>
  #elif defined(HAVE_AIX_OS) || defined(HAVE_WIN32)
...
//                    uint32_t previousCrc32 = 0, size_t prefetchAhead = 256)
...
/* On solaris intel system, in some versions (8.4.0 to 8.4.9), the crc algo was
 * not detecting the endian stuff properly. So we automatically switch from one
 * crc32 to the other one if we detect a mismatch.
 */
...
//  crc = crc32_16bytes_prefetch(data, NumBytes, 0, 256);
  crc = bcrc32((unsigned char *)data, NumBytes);
  duration  = seconds() - startTime;
